import { CheckCircle, AlertTriangle, RefreshCw, Network, Server, Shield } from 'lucide-react'
import { Skeleton } from '../../ui/Skeleton'
import { useCniStatus } from './useCniStatus'
...
export function CniStatus() {
  ...
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useCniStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch CNI status' : 'No CNI plugins found'}
        </p>
        <p className="text-xs">Ensure CNI is configured in your cluster.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">CNI not detected</p>
        <p className="text-xs text-center max-w-xs">
          Configure CNI plugins for pod networking.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex gap-3">
        <MetricTile
          label="Nodes"
          value={`${data.nodesWithCni}/${data.totalNodes}`}
      ...
      <div className="flex-1 flex flex-col gap-2">
        <p className="text-xs font-medium text-muted-foreground">CNI Plugins</p>
        <div className="space-y-1.5">
          {data.plugins.slice(0, 4).map((plugin) => (
            <div key={plugin.name} className="flex items-center gap-2 text-xs">
              ...
              />
              <span className="truncate flex-1 text-muted-foreground">{plugin.name}</span>
              <span className="text-muted-foreground/60 shrink-0">v{plugin.version}</span>
            </div>
          ))}
        </div>
      </div>
