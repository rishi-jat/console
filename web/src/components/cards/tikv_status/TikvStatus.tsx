import { CheckCircle, AlertTriangle, RefreshCw, Database, HardDrive, Layers } from 'lucide-react'
import { Skeleton } from '../../ui/Skeleton'
import { useTikvStatus } from './useTikvStatus'
...
export function TikvStatus() {
  ...
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useTikvStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch TiKV status' : 'No TiKV cluster found'}
        </p>
        <p className="text-xs">Deploy TiKV for distributed key-value storage.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">TiKV not detected</p>
        <p className="text-xs text-center max-w-xs">
          Deploy TiKV for distributed transactional key-value storage.
        </p>
      </div>
    )
  }
    ...
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex gap-4 text-xs">
        <span className="text-muted-foreground">
          PD: {data.pdPods.ready}/{data.pdPods.total}
        </span>
        <span className="text-muted-foreground">
          TiKV: {data.tikvPods.ready}/{data.tikvPods.total}
        </span>
        <span className="text-muted-foreground">Capacity: {data.totalCapacity}</span>
      </div>

      {/* Store list */}
      <div className="flex-1 flex flex-col gap-2">
        <p className="text-xs font-medium text-muted-foreground">TiKV Stores</p>
        <div className="space-y-1.5">
          {data.stores.slice(0, 4).map((store) => (
            <div key={store.id} className="flex items-center gap-2 text-xs">
              ...
              />
              <span className="truncate flex-1 text-muted-foreground">{store.address}</span>
              <span className="text-muted-foreground/60 shrink-0">{store.regionCount} regions</span>
            </div>
          ))}
        </div>
      </div>
