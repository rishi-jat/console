import { CheckCircle, AlertTriangle, RefreshCw, FileSignature, Package, Shield } from 'lucide-react'
...
export function NotaryStatus() {
  const formatRelativeTime = useFormatRelativeTime()
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useNotaryStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch Notary status' : 'No Notary installation found'}
        </p>
        <p className="text-xs">Deploy Notary for image signing and verification.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">Notary not detected</p>
        <p className="text-xs text-center max-w-xs">
          Deploy Notary to enable container image signing and trust.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex-1 flex flex-col gap-2">
        <p className="text-xs font-medium text-muted-foreground">Recent Signatures</p>
        <div className="space-y-1.5">
          {data.signatures.slice(0, 4).map((sig, idx) => (
            <div key={idx} className="flex items-center gap-2 text-xs">
              ...
              <span className="truncate flex-1 text-muted-foreground">
                {sig.repository.split('/').pop()}:{sig.tag}
              </span>
              <span className="text-muted-foreground/60 shrink-0">{sig.signer}</span>
            </div>
          ))}
        </div>
      </div>
