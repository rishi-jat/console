import { CheckCircle, AlertTriangle, RefreshCw, Shield, Archive, Target } from 'lucide-react'
import { Skeleton } from '../../ui/Skeleton'
import { useTufStatus } from './useTufStatus'
...
export function TufStatus() {
  ...
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useTufStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch TUF status' : 'No TUF repositories found'}
        </p>
        <p className="text-xs">Configure TUF for secure software updates.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">TUF not detected</p>
        <p className="text-xs text-center max-w-xs">
          Deploy TUF for secure software update distribution.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex-1 flex flex-col gap-2">
        <p className="text-xs font-medium text-muted-foreground">TUF Repositories</p>
        <div className="space-y-1.5">
          {data.repositories.slice(0, 4).map((repo) => (
            <div key={repo.name} className="flex items-center gap-2 text-xs">
              ...
              />
              <span className="truncate flex-1 text-muted-foreground">{repo.name}</span>
              <span className="text-muted-foreground/60 shrink-0">v{repo.version}</span>
            </div>
          ))}
        </div>
      </div>
