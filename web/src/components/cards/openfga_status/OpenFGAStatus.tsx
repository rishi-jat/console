import { CheckCircle, AlertTriangle, RefreshCw, Users, Database, Activity } from 'lucide-react'
import { Skeleton } from '../../ui/Skeleton'
import { useOpenFGAStatus } from './useOpenFGAStatus'
...
export function OpenFGAStatus() {
  ...
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useOpenFGAStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch OpenFGA status' : 'No OpenFGA installation found'}
        </p>
        <p className="text-xs">Deploy OpenFGA for fine-grained authorization.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">OpenFGA not detected</p>
        <p className="text-xs text-center max-w-xs">
          Deploy OpenFGA for relationship-based access control.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex gap-3">
        ...
        <MetricTile
          label="Tuples"
          value={data.totalTuples.toLocaleString()}
          colorClass="text-green-400"
          icon={<Users className="w-4 h-4 text-green-400" />}
        />
        <MetricTile
          label="QPS"
          value={data.queriesPerSecond.toLocaleString()}
