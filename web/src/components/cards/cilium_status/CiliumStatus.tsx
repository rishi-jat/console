import { CheckCircle, AlertTriangle, RefreshCw, Network, Shield, Eye } from 'lucide-react'
import { Skeleton } from '../../ui/Skeleton'
import { useCiliumStatus } from './useCiliumStatus'
...
export function CiliumStatus() {
  const formatRelativeTime = useFormatRelativeTime()
  const { data, error, showSkeleton, showEmptyState, isRefreshing } = useCiliumStatus()
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm text-red-400">
          {error ? 'Failed to fetch Cilium status' : 'No Cilium installation found'}
        </p>
        <p className="text-xs">Deploy Cilium for eBPF-powered networking.</p>
      </div>
    )
  }
  ...
  if (data.health === 'not-installed') {
    return (
      <div className="h-full flex flex-col items-center justify-center min-h-card text-muted-foreground gap-2">
        ...
        <p className="text-sm font-medium">Cilium not detected</p>
        <p className="text-xs text-center max-w-xs">
          Deploy Cilium for eBPF-based networking and security.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col min-h-card content-loaded gap-4">
      ...
      <div className="flex-1 flex flex-col gap-2">
        <p className="text-xs font-medium text-muted-foreground">Cilium Agents</p>
        <div className="space-y-1.5">
          {data.agents.slice(0, 4).map((agent) => (
            <div key={agent.node} className="flex items-center gap-2 text-xs">
              <span
              ...
              />
              <span className="truncate flex-1 text-muted-foreground">{agent.node}</span>
              <span className="text-muted-foreground/60 shrink-0">{agent.endpoints} eps</span>
            </div>
          ))}
        </div>
      </div>
